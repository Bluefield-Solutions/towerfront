...
5. **`npm run gate`** — achtundzwanzig Prüfungen, rund 90 Sekunden. Alles grün, sonst
...
| 20 | Geländearten | `npm run gelaendetor` | einem unwegsamen Fleck, dessen eingetragene Art oder Farbe nicht zum Kartenbild passt |
| 21 | Lesbarkeit | `npm run lesbarkeit` | zu schwachem Saumkontrast, zu kleiner Silhouette, zu ähnlichen Gegnerfarben |
| 22 | Berührungsflächen | `npm run beruehrung` | Bedienelementen unter 44 Bildschirmpunkten — auf der Leinwand gerechnet, im HTML aus der Stilvorlage gelesen |
| 23 | Bänder über dem Feld | `npm run streifentor` | einer Wellenvorschau über 22 % der Bildhöhe — für **jede** Welle gemessen — oder einer Einweisungsblase über einem Drittel, gemessen am längsten Konter-Satz, den die Ableitung hergibt. Echtes Markup aus `Ui.sync()`, echte Stilvorlage in Chromium; das Browsertor sieht nur die erste, harmloseste Welle |
| 24 | Bildabnahme | `npm run bildtor` | einfarbiger Fläche, falscher Helligkeit, nicht dekodierten Bildern |
| 25 | Rauchtest | `npm run smoke` | Fehlern beim Zeichnen, in der Oberfläche, bei der Eingabe; unerreichbaren Menüwegen; zu kleinen Trefferflächen |
| 26 | Build | `npm run build` | Bündelfehler |
| 27 | Autarkie | `npm run autarkie` | externer URL, nicht inlintem Skript, Safari-Blur-Muster, fehlender DOM-Id, Ersatzschreibung statt Umlaut |
| 28 | Browser | `npm run browser` | Konsolenfehlern; Spielbedienung, die im Menü sichtbar ist; Bedienung, die über der Landkarte liegt; einem Spiel, in das man nicht hineinkommt; Knöpfen, die gerechnet unter 44 Punkten liegen oder verdeckt sind — als einziges Tor mit gerechneter Kaskade, im echten Chromium auf 844 × 390 |
