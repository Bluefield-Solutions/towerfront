...
export class GameState {
  ...
  /** **Welches Vorzeichen ueber der kommenden startbaren Welle steht.**
   *
   *  `kommendes` und nicht `naechstes`: `autarkietor` liest den
   *  AUSGELIEFERTEN Text und faengt Ersatzschreibungen wie „naechste" -
   *  ein verkleinerter Bezeichner steht dort genauso wie eine Aufschrift,
   *  und das Tor kann die zwei nicht trennen. Dritter Fall dieser Klasse
   *  (v292 eine `blurb`-Zeile, v316 ein HTML-Kommentar), und jedes Mal ist
   *  die Reparatur hier und nicht am Tor.
   *
  ...
  get kommendesVorzeichen(): Vorzeichen | null {
